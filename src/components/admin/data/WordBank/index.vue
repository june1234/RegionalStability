<template>
	<div>
		<h2 class="title">世界银行数据</h2>
		<el-form :inline="true" :model="BankFormData" ref="BankFormData">
			<el-form-item prop="nation">
				<el-select v-model="BankFormData.nation" filterable placeholder="选择国家">
					<el-option v-for="item in countries" :key="item.number" :label="item.internetname" :value="item.countrykeys">
					</el-option>
				</el-select>
			</el-form-item>
			<el-form-item prop="startTime">
				<el-date-picker v-model="BankFormData.startTime" type="date" placeholder="选择开始日期"></el-date-picker>
			</el-form-item>
			<el-form-item prop="endTime">
				<el-date-picker v-model="BankFormData.endTime" type="date" placeholder="选择结束日期"></el-date-picker>
			</el-form-item>
			<el-form-item>
				<el-button type="primary" @click="selectGTD">查询</el-button>
			</el-form-item>
		</el-form>
		<el-upload class="avatar-uploader" action="http://192.168.100.202:8080/dev/gtd/import" 
			...
			show-file-list 
			:file-list="fileList">
			<el-button slot="trigger">导入CSV</el-button>
		</el-upload>
		<div class="content">
			<!-- 世界银行 table表格 -->
			<el-table :data="tableBankData" border>
			...
			<!-- 世界银行 表格分页-->
			<el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="BankFormData.pageNum" :page-sizes="[10,20,30]" :page-size="BankFormData.pageSize" layout="total, sizes, prev, pager, next, jumper" :total="total">
			</el-pagination>
		</div>
	</div>
</template>
...
export default {
  ...
  data() {
	...
	methods:{
		//加载世界银行列表
		loadGTDPageList(){
			getGTDList(this.BankFormData).then(res => {
		...
		handleSizeChange(val) {
			this.BankFormData.pageSize=val
			this.loadGTDPageList()
		},
		handleCurrentChange(val) {
			this.BankFormData.pageNum=val
			this.loadGTDPageList()
		},
